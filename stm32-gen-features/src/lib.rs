...
pub struct ChipData {
    name: String,
    cores: Vec<String>,
    reference_manual: Option<String>, // A few chips do not have docs
}

/// Get the list of all the chips' data
///
    ...
    glob::glob("../stm32-data/data/chips/*.json")
        ...
        .filter_map(|entry| {
            if let Some(name) = entry.file_stem().and_then(|stem| stem.to_str()) {
                Some(chip_data_file(&entry, name.to_lowercase()))
            } else {
...
/// Get the chip data from the associated data file.
///
/// # Panic
/// Panics if the file does not exist or if it contains yaml syntax errors.
/// Panics if "cores" is not in the expected format.
/// Panics if "docs" is not in the expected format.
fn chip_data_file(path: &Path, name: String) -> ChipData {
    ...
    let cores = doc["cores"]
        ...
        .collect();
    let reference_manual = doc["docs"]
        .as_vec()
        .unwrap_or_else(|| panic!("{:?}:[docs] is not an array", path))
        .iter()
        .enumerate()
        .find(|(_i, doc)| doc["type"].as_str() == Some("reference_manual"))
        .map(|(i, doc)| {
            doc["display_name"]
                .as_str()
                .unwrap_or_else(|| panic!("{:?}:[docs][{}][display_name] is not a string", path, i))
                .to_lowercase()
        });
    ChipData {
        name,
...
pub fn embassy_stm32_needed_data(data: &[ChipData]) -> String {
    let mut result = String::new();
    let rms = data
        .iter()
        .filter_map(|c| c.reference_manual.clone())
        .collect::<BTreeSet<_>>();
    for rm in rms {
        writeln!(result, "{rm} = []").ok();
    }
    for chip in data {
        if chip.cores.len() > 1 {
            for core_name in &chip.cores {
                if let Some(rm) = &chip.reference_manual {
                    writeln!(
                        result,
                        "{chip}-{core} = [ \"stm32-metapac/{chip}-{core}\", \"{rm}\" ]",
                        chip = chip.name,
                        core = core_name,
                        rm = rm,
                    )
            ...
            if let Some(rm) = &chip.reference_manual {
                writeln!(
                    result,
                    "{chip} = [ \"stm32-metapac/{chip}\", \"{rm}\" ]",
                    chip = chip.name,
                    rm = rm,
                )
                .ok();
            } else {
                writeln!(result, "{chip} = [ \"stm32-metapac/{chip}\"]", chip = chip.name).ok();
            }
        }
    }
    result
...
/// # Panic
/// Panics if a file contains yaml syntax errors or if a value does not have a consistent type
pub fn stm32_metapac_needed_data(data: &[ChipData]) -> String {
    let mut result = String::new();
    for chip in data {
        if chip.cores.len() > 1 {
            for core_name in &chip.cores {
...
mod tests {
    ...
    fn stm32f407vg_yaml_file_exists() {
        assert!(chip_data()
            .as_slice()
            .into_iter()
            .any(|data| { data.name == "stm32f407vg" }))
    }
